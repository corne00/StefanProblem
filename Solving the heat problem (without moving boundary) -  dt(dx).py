...
N_array = np.array([2,4]) # array with number oftime steps for which you want to run the simulation
...
for Mcount, M in enumerate(M_array):
    for Ncount, N in enumerate(N_array):
        ...
        print("Simulation for M =", M, "and N =", N, "and a =", a)
...
if plot_errors is True:   
    ...
    if len(M_array)>2:
        for dtcount, dt in enumerate(dt_array):
            plt.loglog(M_array, errors[:,dtcount], "o--", label= "a = " + str(1/N_array[dtcount]))
